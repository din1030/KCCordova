		...
		if (data.status) {
			$.each(data.result, function(idx, obj) {
				...
				var fav_div = $('<div></div>').addClass('detail_block msg_block').attr('data-type', obj.type).attr('data-user-id', user_id)
					.append('<div class="ui-bar ui-bar-a"><sapn>' + type_word + '</sapn><button class="ui-btn ui-corner-all ui-btn-inline ui-mini ui-btn-right ui-icon-delete ui-btn-icon-notext del-fav-btn" type="button" name="button" data-fav-id="' + obj.fav_id + '" data-shadow="false">x</button></div><a href="' + link + '" data-ajax="false"><div class="ui-body ui-body-a"><div class="avatar msg-avatar float-left"><img src="' + img_base + obj.thumb + '" style="width: 70px; height: 70px; border-radius: 50%;" alt=""></div><strong>' + name + '</strong><br>' + obj.country + '｜' + obj.area + '</div></a>');
			...
			$('<p style="text-align: center; background-color:white; padding:15px;">' + data.message + '</p>').appendTo($('[data-role="main"]'));
		}
